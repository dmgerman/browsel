...
The central elisp entry point for invoking a Chrome handler.
Synchronous; blocks until the Chrome extension responds or the
=chrome-server-request-timeout= deadline (default 10 seconds) expires.
...
| Argument  | Type                                                              |
|-----------+-------------------------------------------------------------------|
| =NAME=    | string — the WebSocket request name (e.g. ="EVAL_IN_ACTIVE_TAB"=) |
| =PAYLOAD= | optional plist — keys depend on the request                       |

Returns a response plist with =:status= (either ="ok"= or ="error"=)
and a request-specific body.  On error, =:message= contains a
human-readable reason.

For =EVAL_IN_ACTIVE_TAB= the payload keys are:

| Key       | Meaning                                                              |
|-----------+----------------------------------------------------------------------|
| =:code=   | string — the JavaScript to evaluate                                  |
| =:tabId=  | optional integer — target tab (default: the active tab in the current window) |
| =:world=  | optional string — ="MAIN"= (page context, default) or ="USER_SCRIPT"= (isolated) |

The response =:result= is an array of =InjectionResult= plists, one
per frame the script ran in.  Each has =:documentId=, =:frameId=, and
=:result= (the value of the last JavaScript expression).
...
Returns the request id immediately; invokes =CALLBACK= with the
response plist when it arrives.  Safe to invoke from any context,
including inside a WebSocket callback.
...
The first request to a tab displays the consent overlay.  Extracting
the value from a one-frame result:
...
The =chrome-server-babel= module defines =org-babel-execute:chrome-js=
as a convenience layer over =chrome-server-request=.  The block body
is the =:code= payload; the value of the last expression becomes the
block result.
