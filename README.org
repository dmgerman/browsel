...
=chrome-server= integrates Emacs with the Chrome browser, in both
directions.  Right-click on a page in Chrome and have it land in
Emacs (org-capture, save-page, archive a ChatGPT conversation, catch
a YouTube video for later).  Going the other way: from Emacs ask the
browser things (what tabs are open?  what's the page title?  what's
the video's current time?) and act on them.

=chrome-server= is composed of two parts:

1. *An Emacs side* (the elisp modules in this repository).  Emacs
   runs a WebSocket server on =127.0.0.1:9130= that exposes named
   handlers for browser-initiated actions and lets you call into the
   browser from elisp.
2. *A Chrome extension* (the =extension/= subdirectory).  An MV3
   extension that dials the Emacs server, paints the right-click
   menus, the toolbar icon, and the popup, and brokers requests in
   both directions.

Both must be running for anything to work.  The Chrome extension
auto-reconnects, so starting / restarting Emacs is non-disruptive —
the popup just blinks from Connected to Connecting and back.  Setup
for each side is below.
...
- Right-click menu items in Chrome that fire Emacs actions:
  org-capture, org-roam-capture, open-in-eww, save-page, save-chatgpt,
...
- Per-tab consent gating arbitrary JavaScript evaluation in the
  browser, surfaced by an in-page overlay and a red toolbar icon while
  active.
- Org-Babel =chrome-js= source blocks that run JavaScript in the
  active tab and return the value into your org buffer.
- An elisp API to query and steer the browser: list tabs, focus,
  open, close, evaluate JS with a return value.
- Adding a new action is a single =config.json= edit + =make build=
...
The first =EVAL_IN_ACTIVE_TAB= request on a tab paints a small overlay
showing the code about to run, with four buttons: *Deny*, *Allow
once*, *Allow 1 hour*, *Allow this tab*.  30 seconds of no answer
counts as deny.
...
The first time, the page paints a consent overlay — read the code,
click Allow.  Same thing from elisp:
